...
class ProjectActivityType(models.Model):
    ...
    def type_progress_rating(self):
        activity_types = self.search([('checklist_ids','!=',False),('checklist_ids.checklist_mark','>',0),('checklist_ids.is_pass','in',['yes','nop'])])
        updated_activity = []
        for rec in activity_types:
            ...
            obtain_rate = sum(checklist_ids.filtered(lambda x: x.is_pass in ['yes','no'] and not x.first_time_check).mapped('checklist_mark'))
            if total_rate and obtain_rate:
                rec.act_type_rating = (obtain_rate / total_rate) * 10
                updated_activity.append(rec.activity_id.id)
        if updated_activity:
            updated_activity_ids = self.env['project.activity'].browse(updated_activity)
...
class ProjectActivity(models.Model):
    ...
    def check_act_progress_rating(self):
        ...
        for rec in self:
            rec.act_rating = 0
            activity_types = rec.activity_type_ids.filtered(lambda obj: obj.status not in ['draft', 'submit'])
            if activity_types:
        ...
        if updated_floors:
            updated_floors_ids = self.env['project.floors'].browse(updated_floors)
...
class ProjectFloors(models.Model):
    ...
    def check_floors_progress_rating(self):
        ...
        for rec in self:
            ...
            if activities:
                total_rate = len(activities.filtered(lambda obj: obj.act_rating != 0.0)) * 10
        ...
        if updated_towers:
            updated_towers_ids = self.env['project.tower'].browse(updated_towers)
    ...
    def check_tower_progress_rating(self):
        ...
        for rec in self:
            ...
            if tower_floor_line_id or tower_flat_line_id:
                obt_rate = sum(tower_floor_line_id.mapped('floors_rating')) + sum(tower_flat_line_id.mapped('flats_rating'))
                total_len = len(tower_floor_line_id.filtered(lambda obj: obj.floors_rating != 0.0)) + len(tower_flat_line_id.filtered(lambda obj: obj.flats_rating != 0.0))
        ...
        if updated_projects:
            updated_projects_ids = self.env['project.info'].browse(updated_projects)
...
class ProjectInfoTowerLineTemp(models.Model):
    ...
    project_tower_rating = fields.Float(string="Project Tower Rating",related='tower_id.tower_rating')
    ...
    def check_project_progress_rating(self):
        for rec in self:
            ...
            if project_info_tower_line_temp:
                total_rate = len(project_info_tower_line_temp.filtered(lambda obj: obj.project_tower_rating != 0.0)) * 10
                obt_rate = sum(project_info_tower_line_temp.mapped('project_tower_rating'))
                if obt_rate and total_rate:
