...
class TestCode(models.Model):
    ...
    def create_mir_seq_no(self):
        mis_model = self.env['material.inspection']
        seq_model = self.env['material.inspection.seq']
        project_ids = self.env['project.info'].search([])
        count = 0

        for project in project_ids:
            records = mis_model.search([('project_info_id', '=', project.id)], order='create_date')
            ...
            for rec in records:
                ...
                financial_year = f"{year}_{str(year + 1)[-2:]}"  # e.g., 2025_26

                # Get or create the seq record
                seq_rec = seq_model.search([
                    ('project_info_id', '=', project.id),
                    ('current_month', '=', month_str)
                ], limit=1)

                if not seq_rec:
                    seq_rec = seq_model.create({
                        'project_info_id': project.id,
                        'current_month': month_str,
                        'seq_no': '0001'
                    })
                ...
                # Assign test_no
                rec.mir_no = f"{project_name}/MI Report/{month_str}/{financial_year}/{seq}"

                # Increment and update the sequence number
                next_seq_int = int(seq) + 1
                seq_rec.seq_no = f"{next_seq_int:04d}"

        _logger.info("--Total records processed for test_no: %s", count)



    # def create_mir_seq_no(self):
    #     mis_seq = self.env['material.inspection.seq']
    #     mis_model = self.env['material.inspection']
    #     project_ids = self.env['project.info'].search([])
    #     count = 0
    #     for project_id in project_ids:

    #         records = mis_model.search([('project_info_id', '=', project_id.id)])

    #         for idx, rec in enumerate(records, start=1):
    #             count+=1
    #             # Project name from related project
    #             project_name = rec.project_info_id.name

    #             # Extract month from create_date (e.g., "May")
    #             month_str = rec.create_date.strftime("%B")

    #             # Simplified financial year: current year + next year
    #             year = rec.create_date.year
    #             financial_year = f"{year}_{str(year + 1)[-2:]}"  # e.g., 2025_26

    #             # Sequence number: 4-digit padded
    #             seq = f"{idx:04d}"

    #             # Final string
    #             rec.test_no = f"{project_name}/MI Report/{month_str}/{financial_year}/{seq}"
    #     _logger.info("--count-----: %s",(count))
    #         #result.append(report_string)
        #return result
        # # Get all material inspection records grouped by project_id
        # project_info_ids = mis_model.search([]).mapped('project_info_id.id')

        # for project_id in project_info_ids:
        #     # Get all records for this project that don't have test_no yet
        #     mis_records = mis_model.search([
        #         ('project_info_id', '=', project_id),
        #         ('test_no', '=', False)
        #     ])

        #     for record in mis_records:
        #         # Extract month and year from the record's create_date
        #         create_dt = record.create_date
        #         record_month = create_dt.strftime('%B')  # Full month name (e.g., "May")
        #         short_month = create_dt.strftime('%b')   # Short month (e.g., "May")
        #         year = create_dt.year
        #         next_year = year + 1
        #         year_str = f"{year}_{str(next_year)[-2:]}"  # e.g., "2024_25"

        #         # Get or create a sequence record based on project_id and record_month
        #         seq_rec = mis_seq.search([
        #             ('project_info_id', '=', project_id),
        #             ('current_month', '=', record_month)
        #         ], limit=1)

        #         if not seq_rec:
        #             seq_rec = mis_seq.create({
        #                 'project_info_id': project_id,
        #                 'current_month': record_month,
        #                 'seq_no': '0001'
        #             })

        #         # Generate the current sequence number
        #         project_name = seq_rec.project_info_id.name or self.env['project.info'].browse(project_id).name
        #         seq = seq_rec.seq_no

        #         # Format test_no
        #         test_no = f"{project_name}/MI Report/{short_month}/{year_str}/{seq}"
        #         record.test_no = test_no

        #         # Increment and update the sequence
        #         int_seq = int(seq)
        #         int_seq += 1
        #         seq_rec.seq_no = f'{int_seq:04}'


    # def create_mir_seq_no(self):
    #     mis_seq = self.env['material.inspection.seq']
    #     mis = self.env['material.inspection']
    #     records = mis.search([])
    #     project_info_ids = records.mapped('project_info_id.id')

    #     _logger.info("---grouped_datagrouped_data-------: %s",(project_info_ids))
    #     for project_id in project_info_ids:
    #         mis_records = mis.search([('project_info_id','=',project_id)])
    #         _logger.info("--mis_records------: %s",(mis_records))
