...
class VJDBuHierarchy(models.Model):
    ...
    @api.model
    def action_get_vjd_bu_hierarchy1(self, timestamp):
        bu_rec = self.env['business.unit'].search([], order='id asc', limit=18)
        response = self.action_get_vjd_bu_hierarchy(timestamp,bu_rec)
        return response

    @api.model
    def action_get_vjd_bu_hierarchy2(self, timestamp):
        all_recs = self.env['business.unit'].search([], order='id asc')
        bu_rec = all_recs[18:]
        response = self.action_get_vjd_bu_hierarchy(timestamp,bu_rec)
        return response
        
    @api.model
    def action_get_vjd_bu_hierarchy(self, timestamp,bu_rec):
                ...
                if response.status_code == 200:
                    ...
                    for unit in data:
                        try:
                            ...
                            error_count += 1
                            error = error +" 1## "+ str(response.text)
                    ...
                    error_count += 1
                    error = error +" 2## "+ str(response.text)
