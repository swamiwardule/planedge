...
class ProjectInfo(models.Model):
            ...
            for project in projects:
                ...
                for matched_project in matched_projects:
                    ...
                    project_details.append({
                        'name': matched_project.project_info_id.name,
                        'nc_count': nc_count,
                        'yellow_flag_count': yellow_flag_count,
                        'orange_flag_count': orange_flag_count,
                        'red_flag_count': red_flag_count,
                ...
                count_list_nc = {r['name']: r['nc_count'] for r in project_details}
                count_list_yc = {r['name']: r['yellow_flag_count'] for r in project_details}
                count_list_oc = {r['name']: r['orange_flag_count'] for r in project_details}
                count_list_rc = {r['name']: r['red_flag_count'] for r in project_details}
                count_list_gc = {r['name']: r['green_flag_count'] for r in project_details}
    ...
    def get_c_m_a_data(self, project_id, tower, project_detailsValue):
        ...
        if project_detailsValue == "Material Inspection":
            # If Material Inspection, fetch from material.inspection
            MaterialInspection = self.env['material.inspection']
            return MaterialInspection.get_completed_report_counts(project_id, tower)
    ...
    @api.model
    def get_pending_c_m_a_data(self, project_id, tower,project_detailsValue):

        if project_detailsValue == "Material Inspection":
            # If Material Inspection, fetch from material.inspection
            MaterialInspection = self.env['material.inspection']
            return MaterialInspection.get_pending_report_counts(project_id, tower)
        ...
        for activity_type in ['Maker', 'Checker', 'Approver']:
            ...
            graph_result.append(
                {'l_month': activity_type, 'leave': activity_counts})
        _logger.info('-----111graph____result------%s', graph_result)
...
class ProjectTower(models.Model):
    ...
    def get_all_project_towers(self):
        try:
            ...
            total_tower = 0
            error_message = _("An error occurred while retrieving project tower count: %s") % str(e)
            # Log the error or handle it appropriately
            _logger.error(error_message)
        return {
    ...
    def get_tower_names(self, projectValue, project_detailsValue):
        #print('=========projectValue=========project_detailsValue---', projectValue, project_detailsValue)
...
class MaterialInspection(models.Model):
    _inherit = "material.inspection"
  
    @api.model
    def get_pending_report_counts(self, project_id=None, tower=None):
        project_info_id = project_id
        tower_id = tower
        domain = []
        if project_id:
            domain.append(('project_info_id', '=', int(project_info_id)))
        if tower:
        ...
        for rec in status_counts:
            ...
            if status == 'draft':
                ...
                result['Checker'] += count
            elif status == 'checked':
                result['Approver'] += count
        ...
        name = ''
        if tower:
            name = self.env['project.tower'].browse(int(tower_id)).name or ''
        elif project_id:
            name = self.env['project.project'].browse(int(project_info_id)).name or ''

        graph_result = [
            {'l_month': 'Maker', 'leave': {name: result['Maker']}},
            {'l_month': 'Checker', 'leave': {name: result['Checker']}},
            {'l_month': 'Approver', 'leave': {name: result['Approver']}},
        ]

        _logger.info("-----111graph_____result------%s", graph_result)
        return [graph_result, [name]]
    ...
    @api.model
    def get_completed_report_counts(self, project_id=None, tower=None):
        project_info_id = project_id
        tower_id = tower
        domain = []
        if project_id:
    ...
    # def get_pending_report_counts(self, project_id=None, tower=None):
    #     """
    #     Returns pending report counts for Maker, Checker, and Approver,
    #     filtered by project_id and tower if provided, along with graph_result.
    #     """
    #     project_info_id = project_id
    #     tower_id = tower
    #     domain = []
    ...
    #     name = ''
    #     if tower:
    #         name = self.env['project.tower'].browse(int(tower_id)).name
    #     elif project_id:
    #         name = self.env['project.project'].browse(int(project_info_id)).name

    #     graph_result = []
    #     for role in ['maker', 'checker', 'approver']:
    #         graph_result.append({
    #             'l_month': role.capitalize(),
    #             'leave': {name: result[role]['pending']}
    #         })

    #     print('-----graph_result (pending reports)------', graph_result)
    #     return graph_result
    ...
    #         graph_result.append({
    #             'l_month': role.capitalize(),
    #             'leave': {name: result[role]['completed']}
    #         })

    #     print('-----graph_result (completed reports)------', graph_result)
    #     return graph_result
