...
class ProjectTower(models.Model):
    ...
    activity_ids = fields.One2many('project.activity', 'project_tower_id',string="Common Activities")
    development_activity_ids = fields.One2many('project.activity', 'tower_id',string="Development Activities")
    image = fields.Binary('Image')
    assigned_to_ids = fields.Many2many('res.users',tracking=True)
    hqi_user_ids = fields.Many2many(
        'res.users',
        'tower_user_rel',
        'tower_id',
        'user_id',
    ...
    string="Tower Type",
    readonly=False)
    #hqi_ids = fields.One2many('project.floors','tower_id',string="Tower Floor",tracking=True)
    flat_site_visit_ids = fields.One2many('flat.site.visit','tower_id',string="Flat Site Visit(s)")
    hqi_state = fields.Selection([
        ('draft', 'Draft'),
        ('progress', 'In Progress'),
        ('completed', 'Completed'),
    ], string="HQI State", default='draft', tracking=True)
    ...
    def action_reorder_flats_by_name(self):
        for record in self:
            valid_lines = []
            for line in record.tower_flat_line_id:
                try:
                    flat_number = int(line.name)
                    valid_lines.append((flat_number, line))
                except (ValueError, TypeError):
                    continue  # Skip non-integer names

            sorted_lines = sorted(valid_lines, key=lambda x: x[0])
            for i, (_, line) in enumerate(sorted_lines, start=1):
                line.sequence = i * 10
...
class ProjectFlats(models.Model):
    ...
    flats_progress_percentage = fields.Float(string="Progress",store=True)
    flat_hqi_progress_percentage = fields.Float(
        string="HQI Progress (%)",
        compute="_compute_flat_hqi_progress_percentage",
        store=True)
    activity_state = fields.Selection(selection=[('draft', 'Draft'), ('created', 'Created')],default='draft',string="Activity State",readonly=False,store=True,tracking=True)
    hqi_state = fields.Selection([
        ('draft', 'Draft'),
        ('progress', 'In Progress'),
        ('completed', 'Completed'),
    ...
    flat_site_visit_ids = fields.One2many('flat.site.visit','flat_id',string="Flat Site Visit(s)")
    
    original_observation_count = fields.Integer(
        string="Original Observation Count",
        compute="_compute_original_observation_count",
        store=True
    )

    @api.model
    def get_flat_hqi_offline(self, flat_id, user_id):
        flat_rec = self.search([('id', '=', flat_id)], limit=1)
        user = self.env['res.users'].browse(user_id)
        allowed_groups = [
            'custom_project_management.group_quality_maker',
            'custom_project_management.group_quality_approver'
        ]
        has_maker_access = any(user.has_group(group) for group in allowed_groups)
        master_data = []
        get_param = self.env['ir.config_parameter'].sudo().get_param
        base_url = get_param('web.base.url', default='http://www.odoo.com?NoBaseUrl')

        if flat_rec:
            for visit in flat_rec.flat_site_visit_ids:
                location_list = []
                for location in visit.sv_location_ids:
                    # Gather observations for this location
                    observations = []
                    color_list = []
                    for observation in location.svb_location_ids:
                        color_list.append(observation.color)
                        img_list = []
                    ...
                    # Compute overall color for the location
                    if color_list:
                        ...
                        location_color = location.color or False
                    # Add overall color to each observation
                    ...
                    location_dict = {
                        ...
                        'pending_observation_count': location.pending_observation_count,
                        'completed_observation_count': location.completed_observation_count,
                        'checker_completed_count': location.checker_completed_count,
                        'checker_pending_count': location.checker_pending_count,
                        'maker_completed_count': location.maker_completed_count,
                        'maker_pending_count': location.maker_pending_count,
                        'observations': observations,
                        'location_overall_color': location_color,
                    }
                    if has_maker_access:
                        if location.svb_location_ids:
                ...
                visit_dict = {
                    ...
                    'project_name': visit.project_id.name,
                    'flat_id': flat_rec.id,
                    'flat_name': flat_rec.name,
                    'tower_id': visit.tower_id.id,
                    'tower_name': visit.tower_id.name,
                    'sequence': visit.sequence,
                    'color': visit.color,
                    'total_observation_count': visit.total_observation_count,
                    'pending_observation_count': visit.pending_observation_count,
                    'completed_observation_count': visit.completed_observation_count,
                    'location_data': location_list,
                    'checker_completed_count': visit.checker_completed_count,
                    'checker_pending_count': visit.checker_pending_count,
                    'maker_completed_count': visit.maker_completed_count,
                    'maker_pending_count': visit.maker_pending_count,
                }
                master_data.append(visit_dict)
        return master_data
    ...
    #             master_data.append(visit_dict)
    #     #_logger.info("---master_data-------%s",(master_data))
        
    #     return master_data
        # flats_rec = self.browse(flat_id)
        # flat_data = []
        # for flat in flats_rec:
        #     for visit in flat.flat_site_visit_ids:
        #         visit_dict = {
        #             'visit_id': visit.id,
        #             'visit_name': visit.name,
        #             'color': visit.color,
        ...
        #                 'observations': []
        #             }
        #             for obs in location.svb_location_ids:
        #                 obs_dict = {
        #                     'observation_id': obs.id,
        #                     'name': obs.name,
        #                     'state': obs.state,
        #                     'color': obs.color,
        #                     'date': obs.date.isoformat() if obs.date else '',
        #                     'target_date': obs.target_date.isoformat() if obs.target_date else '',
        #                     'issue_category_id': obs.issue_category_id.id if obs.issue_category_id else False,
        #                     'issue_category_name': obs.issue_category_id.name if obs.issue_category_id else '',
        #                     'issue_type_id': obs.issue_type_id.id if obs.issue_type_id else False,
        #                     'issue_type_name': obs.issue_type_id.name if obs.issue_type_id else '',
        #                     'description': obs.description or '',
        #                     'remark': obs.remark or '',
        #                     'impact': obs.impact or '',
        #                     'checker_submitted': obs.checker_submitted,
        #                     'maker_submitted': obs.maker_submitted,
        #                     'is_original_observation': obs.is_original_observation,
        #                 }
        #                 location_dict['observations'].append(obs_dict)
        #             visit_dict['locations'].append(location_dict)
        #         flat_data.append(visit_dict)
        # return flat_data




    def get_flat_hqi_pdf(self,flat_id):
        # Ensure correct user context
        record = self.sudo().browse(flat_id)
        ...
        try:
            ...
            report = self.env.ref('custom_project_management.action_report_flat_hqi').sudo()
    ...
    def action_generate_flat_hqi_report(self):
        return self.env.ref('custom_project_management.action_report_flat_hqi').report_action(self)

    @api.depends(
        'flat_site_visit_ids.total_observation_count',
        'flat_site_visit_ids.completed_observation_count'
    )
    def _compute_flat_hqi_progress_percentage(self):
        for flat in self:
            total = sum(flat.flat_site_visit_ids.mapped('total_observation_count'))
            completed = sum(flat.flat_site_visit_ids.mapped('completed_observation_count'))
            if total > 0:
                flat.flat_hqi_progress_percentage = round((completed / total) * 100, 2)
            else:
                flat.flat_hqi_progress_percentage = 0.0

    @api.depends('flat_site_visit_ids.total_observation_count')
    def _compute_original_observation_count(self):
        for flat in self:
            flat.original_observation_count = sum(
                visit.total_observation_count for visit in flat.flat_site_visit_ids
            )
    ...
    def create(self, values):
        ...
        main_model.project_activity_name_id
        main_model.tower_id
        return main_model
    ...
    @api.model
    def get_hqi_flats(self, project_id, tower_id):
        flats_rec = self.search([
        ...
        flat_data = []
        for flat in flats_rec:
            flat_data.append({'flat_hqi_progress_percentage':flat.flat_hqi_progress_percentage,'flat_id':flat.id,'flat_name':flat.name,'hqi_state':flat.hqi_state})
        return flat_data


    # @api.model
    # def get_flat_hqi_offline(self, flat_id):
    #     flats_rec = self.browse(flat_id)
    #     flat_data = []
    #     for flat in flats_rec:
    #         for visit in flat.flat_site_visit_ids:
    ...
    #                     'sub_locations': []
    #                 }
    #                 for sub_location in location.svb_location_ids:
    #                     sub_location_dict = {
    #                         'sub_location_id': sub_location.id,
    #                         'sub_location_name': sub_location.name,
    #                         'sub_unit_type': sub_location.unit_type,
    #                         'sub_color': sub_location.color,
    #                         'sub_observation_count': sub_location.observation_count,
    #                         'sub_pending_observation_count': sub_location.pending_observation_count,
    #                         'sub_completed_observation_count': sub_location.completed_observation_count,
    #                         'sub_checker_completed_count': sub_location.checker_completed_count,
    #                         'sub_checker_pending_count': sub_location.checker_pending_count,
    #                         'sub_maker_completed_count': sub_location.maker_completed_count,
    #                         'sub_maker_pending_count': sub_location.maker_pending_count,
    #                     }
    #                     location_dict['sub_locations'].append(sub_location_dict)
    #                 visit_dict['locations'].append(location_dict)
    #             flat_data.append(visit_dict)
    #     return flat_data

    @api.model
    def complete_flat_hqi(self, flat_id):
        flat_rec = self.browse(flat_id)
        if flat_rec.exists():
            flat_rec.hqi_state = 'completed'
        return
    
    @api.model
    def get_flat_visits(self, project_id, tower_id, flat_id, user_id):
        flat_rec = self.search([('id', '=', flat_id)], limit=1)
        user = self.env['res.users'].browse(user_id)
        allowed_groups = [
            'custom_project_management.group_quality_maker',
...
class ProjectActivity(models.Model):
    ...
    def action_submit_checklist(self):
        active_ids = self.env.context.get('active_ids', [])
        if not active_ids:
            _logger.warning("No records selected for submission.")
            return
        records = self.browse(active_ids).filtered(lambda r: r.status_code == 400)
        for record in records:
            data_str = record.data.replace("'", '"')  # naive conversion
            data_str = data_str.replace("None", "null")  # convert Python None to JSON null if present

            try:
                data_dict = json.loads(data_str)
                response = self.submit_checklist(data_dict)
    ...
    def submit_checklist(self,data):
        url = "https://qualityapi.vjerp.com/vj-submit-checklist/"
        headers = {
            'Authorization': 'apikey eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjo0ODk2NzQ2NjM4LCJpYXQiOjE3NDMxNDY2MzgsImp0aSI6ImZkZDIwNWRmZTQxMDRiMGE4NmY5MDVmYzQ3YTA5ZThkIiwidXNlcl9pZCI6Mn0.6LjmLkNrK-RIljAQqhDtgB_48pEphnyRiynZnX0lnnw',
...
class ProjectActivityType(models.Model):
    ...
    progress_percentage = fields.Float(string="Progress")
    project_id = fields.Many2one('project.info',string='Project',compute="_compute_project_id",store=True)
    flat_id = fields.Many2one(related="activity_id.flat_id",string='Flat',store=True)
    floor_id = fields.Many2one(related="activity_id.floor_id",string='Floor')
    tower_id = fields.Many2one('project.tower',string='Tower',compute="_compute_tower_id")
    ...
    def write(self, vals):
        """Optimize batch updates by recalculating activity status only when needed."""
        res = super(ProjectActivityType, self).write(vals)
        if 'status' in vals and vals['status'] == 'approve':
            # Check if all activity types linked to this activity are approved
            if self.activity_id:
                all_approved = all(rec.status == 'approve' for rec in self.activity_id.activity_type_ids)
                if all_approved:
                    self.activity_id.write({'state': 'completed','completed_date':fields.Datetime.now().date()})
        return res

    @api.model
    def get_c_m_a_data(self, project_id, tower, project_detailsValue=False):
        domain = []
        if tower:
            ...
            project_list = self.env['project.info'].project_name_list(project_id, tower)
    ...
    def action_submit(self):
        ...
        records.write({'status': 'submit', 'submitted_date': fields.Datetime.now()})
    ...
    @api.depends('activity_id.floor_id', 'activity_id.flat_id', 'activity_id.tower_id')
    def _compute_tower_id(self):
        value = self.env['ir.config_parameter'].sudo().get_param('custom_project_management.on_off_value', default=False)
        if value:
            for rec in self:
                if rec.activity_id:
                    if rec.activity_id.floor_id and rec.activity_id.floor_id.tower_id:
                        ...
                        rec.tower_id = rec.activity_id.flat_id.tower_id.id
                    elif rec.activity_id.tower_id:
                        rec.tower_id = rec.activity_id.tower_id.id
                    else:
                        rec.tower_id = False
                else:
                    rec.tower_id = False
            ...
            for rec in self:
                rec.tower_id = rec.tower_id or False
    ...
    def button_submit(self, seq_no=None, user_id=None):
        _logger.info("----------button_submit---------, %s, %s, %s", self, seq_no, user_id)
        ...
        if user_id:
            ...
            if self.tower_id.assigned_to_ids:
                failed_log['assigned_ids'] = self.tower_id.assigned_to_ids.ids

                for assigned_user in self.tower_id.assigned_to_ids:
                    # Send notification only if user is a checker
                    if assigned_user.has_group('custom_project_management.group_quality_checker'):
                        player_id, _ = self.env['res.users'].get_player_id(assigned_user.id)
                        message = f"CheckList No {seq_no} Submitted by {user_record.name}"
                        title = f"{user_record.name} Submitted the Checklist"
                        ...
                        try:
                            if player_id:
                                ...
                                _logger.warning("Missing player ID for user ID %s", assigned_user.id)
                                failed_log.setdefault('missing_players', []).append(assigned_user.id)
                        except Exception as e:
                            _logger.error("Error sending notification: %s", str(e))
                            failed_log.setdefault('exceptions', []).append({
                                'user_id': assigned_user.id,
                                'error': str(e)
                            })
                    else:
                        failed_log.setdefault('not_checker_users', []).append(assigned_user.id)
            else:
                failed_log['assigned_ids_missing'] = True

        # if not sent and not log_id:
        #     notification_log_obj.create({
        #         'title': str(failed_log),
        #         'status': 'failed'
        #     })
    ...
    #             for assigned_user in self.tower_id.assigned_to_ids:
    #                 # Send only to users in Approver group
    #                 if assigned_user.has_group('custom_project_management.group_quality_approver'):  # Replace with actual XML ID
    #                     player_id, _ = self.env['res.users'].get_player_id(assigned_user.id)
    #                     message = f"CheckList No {seq_no} Submitted by {user_record.name}"
    #                     title = f"{user_record.name} Submitted the Checklist"
    ...
    #                         })
    #                 else:
    #                     failed_log.setdefault('not_approvers', []).append(assigned_user.id)
    #         else:
    #             failed_log['assigned_ids_missing'] = True
    ...
    #         'seq_no': seq_no,
    #         'user_id': user_id,
    #         'method': 'button_set_to_maker'
    #     }

    #     if user_id:
    #         user_record = self.env['res.users'].browse(user_id)
    #         message = f"CheckList No {seq_no} Rejected by {user_record.name}"
    #         title = f"{user_record.name} Rejected the Checklist"

    #         _logger.info("---button_set_to_maker----self.user_maker--------, %s", self.user_maker)

    #         # If a specific maker is assigned, send notification directly to them
    #         if self.user_maker:
    #             player_id, _ = self.env['res.users'].get_player_id(self.user_maker.id)
    #             try:
    #                 if player_id:
    #                     log_id = notification_obj.send_push_notification(
    #                         title,
    ...
    #                     sent = True
    #                 else:
    #                     failed_log.update({'player_id': None, 'user_id': self.user_maker.id, 'reason': 'Missing player_id'})
    #             except Exception as e:
    #                 _logger.error("--wi2--Exception sending to user_maker: %s", str(e))
    #                 failed_log.setdefault('exceptions', []).append({'user_id': self.user_maker.id, 'error': str(e)})

    #         # If no specific maker assigned, loop through assigned users and notify makers
    #         elif self.tower_id.assigned_to_ids:
    #             failed_log['assigned_ids'] = self.tower_id.assigned_to_ids.ids

    #             for assigned_user in self.tower_id.assigned_to_ids:
    #                 if assigned_user.has_group('custom_project_management.group_quality_maker'):  # Replace with correct group XML ID
    #                     player_id, _ = self.env['res.users'].get_player_id(assigned_user.id)
    #                     try:
    #                         if player_id:
    #                             log_id = notification_obj.send_push_notification(
    ...
    #                             failed_log.setdefault('missing_players', []).append(assigned_user.id)
    #                     except Exception as e:
    #                         _logger.error("--wi1--Exception sending to assigned_user: %s", str(e))
    #                         failed_log.setdefault('exceptions', []).append({'user_id': assigned_user.id, 'error': str(e)})
    #                 else:
    #                     failed_log.setdefault('not_makers', []).append(assigned_user.id)
    #         else:
    #             failed_log['assigned_ids_missing'] = True

    #     if not sent and not log_id:
    #         notification_log_obj.create({'title': str(failed_log), 'status': 'failed'})
    ...
    def button_set_to_checker(self, seq_no, user_id):
        ...
        if user_id:
            ...
            if self.user_checker:
                # Send notification directly to assigned checker
                player_id, _ = self.env['res.users'].get_player_id(self.user_checker.id)
                if player_id:
                    try:
                        log_id = notification_obj.send_push_notification(
                            title,
                        ...
                        _logger.error("--Exception sending to user_checker: %s", str(e))
                        failed_log.setdefault('exceptions', []).append({
                            'user_id': self.user_checker.id,
                            'error': str(e)
                        })
                else:
                    failed_log.update({
                ...
                # No user_checker set, fallback to users in Checker group
                assigned_users = self.project_id.assigned_to_ids
                if assigned_users:
                    failed_log['assigned_ids'] = assigned_users.ids

                    for user in assigned_users:
                        if user.has_group('custom_project_management.group_quality_checker'):  # Replace with actual XML ID
                            ...
                            if player_id:
                                try:
                                    ...
                                    _logger.error("--Exception sending to fallback checker: %s", str(e))
                                    failed_log.setdefault('exceptions', []).append({
                                        'user_id': user.id,
                                        'error': str(e)
                                    })
                            else:
                                failed_log.setdefault('missing_players', []).append(user.id)
                        else:
                            failed_log.setdefault('not_checkers', []).append(user.id)
                else:
                    failed_log['assigned_ids_missing'] = True
