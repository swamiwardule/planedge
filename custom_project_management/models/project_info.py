...
class ProjectInfo(models.Model):
    ...
    nc_count = fields.Integer(compute="_compute_total_flag_count_from_tower", compute_sudo=True, store=True)
    yellow_flag_count = fields.Integer(compute="_compute_total_flag_count_from_tower", compute_sudo=True, store=True)
    orange_flag_count = fields.Integer(compute="_compute_total_flag_count_from_tower", compute_sudo=True, store=True)
    red_flag_count = fields.Integer(compute="_compute_total_flag_count_from_tower", compute_sudo=True, store=True)
    green_flag_count = fields.Integer(compute="_compute_total_flag_count_from_tower", compute_sudo=True, store=True)
    ...
    activity_draft_count = fields.Integer(string='Draft Activities', compute='_compute_activity_counts', compute_sudo=True, store=True)
    activity_submit_count = fields.Integer(string='Submit Activities', compute='_compute_activity_counts', compute_sudo=True, store=True)
    activity_approved_count = fields.Integer(string='Approved Activities', compute='_compute_activity_counts', compute_sudo=True, store=True)
    activity_checked_count = fields.Integer(string='Checker Activities', compute='_compute_activity_counts', compute_sudo=True, store=True)

    maker_user_ids = fields.Many2many('res.users', 'maker_id', string='Maker', compute='_compute_maker_id', compute_sudo=True, store=True)
    checker_user_ids = fields.Many2many('res.users', 'checker_id', string='Checker', compute='_compute_checker_id', compute_sudo=True, store=True)
    approver_user_ids = fields.Many2many('res.users', 'approver_id', string='Approver', compute='_compute_approver_id', compute_sudo=True, store=True)
    ...
    maker_pending_count = fields.Integer(string="Maker Pending Count", compute="_compute_c_m_a_counts", compute_sudo=True, store=False, )
    maker_completed_count = fields.Integer(string="Maker Completed Count", compute="_compute_c_m_a_counts", compute_sudo=True, store=True)
    checker_pending_count = fields.Integer(string="Checker Pending Count", compute="_compute_c_m_a_counts",  compute_sudo=True,store=True)
    checker_completed_count = fields.Integer(string="Checker Completed Count", compute="_compute_c_m_a_counts", compute_sudo=True, store=True)
    approver_pending_count = fields.Integer(string="Approver Pending Count", compute="_compute_c_m_a_counts",  compute_sudo=True,store=True)
    approver_completed_count = fields.Integer(string="Approver Completed Count", compute="_compute_c_m_a_counts", compute_sudo=True, store=True)
    ...
    graph_data = fields.Text(string="Graph Data", compute="_compute_graph_data", store=False, compute_sudo=True,)
    project_list = fields.Text(string="Project List", compute="_compute_graph_data", store=False,  compute_sudo=True,)
    active = fields.Boolean(default=True)
    project_hqi_user_ids = fields.Many2many(
    'res.users',
    'project_info_hqi_users_relation',    # new unique relation table name
    'tower_id',
    'user_id',
    string="HQI Users",
    tracking=True
    ...
    # # Converted computed fields to normal fields
    # nc_count = fields.Integer(string="NC Count")
    # yellow_flag_count = fields.Integer(string="Yellow Flag Count")
    # orange_flag_count = fields.Integer(string="Orange Flag Count")
    # red_flag_count = fields.Integer(string="Red Flag Count")
    # green_flag_count = fields.Integer(string="Green Flag Count")
    # project_progress_bar = fields.Float(string="Progress")

    # activity_draft_count = fields.Integer(string='Draft Activities')
    # activity_submit_count = fields.Integer(string='Submit Activities')
    # activity_approved_count = fields.Integer(string='Approved Activities')
    # activity_checked_count = fields.Integer(string='Checker Activities')

    # maker_user_ids = fields.Many2many('res.users', 'maker_id', string='Maker')
    # checker_user_ids = fields.Many2many('res.users', 'checker_id', string='Checker')
    # approver_user_ids = fields.Many2many('res.users', 'approver_id', string='Approver')

    # user_checker = fields.Many2one("res.users", string="Checker")
    # user_approver = fields.Many2one("res.users", string="Approver")
    # project_activity_id = fields.Many2one("project.activity.type")

    # maker_pending_count = fields.Integer(string="Maker Pending Count")
    # maker_completed_count = fields.Integer(string="Maker Completed Count")
    # checker_pending_count = fields.Integer(string="Checker Pending Count")
    # checker_completed_count = fields.Integer(string="Checker Completed Count")
    # approver_pending_count = fields.Integer(string="Approver Pending Count")
    # approver_completed_count = fields.Integer(string="Approver Completed Count")
    # total_cma_pending = fields.Integer(string="Total CMA Pending")
    # total_cma_complete = fields.Integer(string="Total CMA Completed")

    # graph_data = fields.Text(string="Graph Data")
    # project_list = fields.Text(string="Project List")



    # @api.model
    # def run_tower_counts_scheduler(self):
    #     records = self.search([])  # or use a domain to limit projects
    #     records._compute_total_flag_count_from_tower()

    @api.model
    def run_tower_counts_scheduler(self):
        _logger.info("======= Starting Project Tower Count Recalculation =======")

        # First recompute tower-level data
        tower_temp_records = self.env['project.info.tower.line.temp'].search([])
        _logger.info("Found %s tower line temp records", len(tower_temp_records))
        for tower_line in tower_temp_records:
            tower_line._compute_total_flag_count_from_tower()

        # Now recompute project-level data
        project_records = self.search([])
        _logger.info("Found %s projects", len(project_records))
        for project in project_records:
            project._compute_total_flag_count_from_tower()
            _logger.info(
                "Project %s -> Yellow: %s | Orange: %s | Red: %s | Green: %s | NC: %s",
                project.name,
                project.yellow_flag_count,
                project.orange_flag_count,
                project.red_flag_count,
                project.green_flag_count,
                project.nc_count,
            )

        _logger.info("======= Recompute Project Tower Counts Completed =======")
        ...
        if value:
            try:
                for record in self:
                    record.graph_data = ''
                    record.project_list = ''
                    graph_result, project_list = project_activity_type_obj.get_c_m_a_data(record.id, False)
    ...
    def _compute_c_m_a_counts(self):
        ...
        if value:
            for record in self:
                ...
                project_activity_info = project_activity_type_obj.sudo().search([('project_id', '=', record.id)])
...
#added by swami for correct project flag count 
    @api.depends(
        'project_info_tower_line_temp.project_nc',
        'project_info_tower_line_temp.project_yellow',
        'project_info_tower_line_temp.project_orange',
        'project_info_tower_line_temp.project_red',
        'project_info_tower_line_temp.project_green',
    )
    def _compute_total_flag_count_from_tower(self):
        ...
        for record in self:
            # reset first
            record.nc_count = record.yellow_flag_count = record.orange_flag_count = record.red_flag_count = record.green_flag_count = 0
            if not value:
                continue

            # fetch manually.set.flag entries for this project
            flag_records = self.env['manually.set.flag'].sudo().search([
                ('project_info_id', '=', record.id),
                ('status', '=', 'open')
            ])

            if flag_records:
                record.nc_count = sum(int(x.cre_nc or 0) for x in flag_records)
                record.yellow_flag_count = sum(int(x.cre_yellow or 0) for x in flag_records)
                record.orange_flag_count = sum(int(x.cre_orange or 0) for x in flag_records)
                record.red_flag_count = sum(int(x.cre_red or 0) for x in flag_records)
                record.green_flag_count = sum(int(x.cre_Green or 0) for x in flag_records)

            _logger.info(
                "Project %s Flags -> Yellow: %s | Orange: %s | Red: %s | Green: %s | NC: %s",
                record.name,
                record.yellow_flag_count,
                record.orange_flag_count,
                record.red_flag_count,
                record.green_flag_count,
                record.nc_count
            )
...
class ProjectInfoTowerLineTemp(models.Model):
    ...
    def _compute_total_flag_count_from_tower(self):
        ...
        for record in self:
            record.project_nc = record.project_yellow = record.project_orange = record.project_red = record.project_green = 0
            if not value:
                continue
            ...
            # Fetch correct counts using manually.set.flag (like in your working method)
            flag_records = self.env['manually.set.flag'].sudo().search([
                ('project_tower_id', '=', tower.id),
                ('status', '=', 'open')
            ])

            nc_count = sum(int(x.cre_nc or 0) for x in flag_records)
            yellow_flag_count = sum(int(x.cre_yellow or 0) for x in flag_records)
            orange_flag_count = sum(int(x.cre_orange or 0) for x in flag_records)
            red_flag_count = sum(int(x.cre_red or 0) for x in flag_records)
            green_flag_count = sum(int(x.cre_Green or 0) for x in flag_records)

            # Assign totals
            record.project_nc = nc_count
            record.project_yellow = yellow_flag_count
            record.project_orange = orange_flag_count
            record.project_red = red_flag_count
            record.project_green = green_flag_count
    ...
    def _process_activities(self, container, unit_type, project_name, tower_name):
        if unit_type in ['flat', 'floor']:
            for item in container:
                location_name = getattr(item, 'name', False)
                if not location_name:
                    _logger.warning("Missing location_name for unit_type %s in record %s", unit_type, item)
                    continue

                activity_seq = 1
                for activity in getattr(item, 'activity_ids', []):
                    ...
                    for activity_type in activity.activity_type_ids:
                        ...
                        activity_type.seq_no = f"PLANEDGE/{project_name}/{tower_name}/{location_name}/{activity.name}/{activity_type_seq:03d}"
                        activity_type_seq += 1

        elif unit_type in ['common', 'development']:
            area = "Common Area" if unit_type == 'common' else ""
            activity_seq = 1
            for activity in container:
                ...
                for activity_type in activity.activity_type_ids:
                    activity_type.index_no = f"{activity_type_seq:03d}"
                    if area:
                        activity_type.seq_no = f"PLANEDGE/{project_name}/{tower_name}/{area}/{activity.name}/{activity_type_seq:03d}"
                    else:
                        activity_type.seq_no = f"PLANEDGE/{project_name}/{tower_name}/{activity.name}/{activity_type_seq:03d}"
                    activity_type_seq += 1

    
    def generate_seq_for_flats(self):
        if not self.tower_id or not self.tower_id.project_id:
            return
        self._process_activities(self.tower_id.tower_flat_line_id, 'flat', self.tower_id.project_id.name, self.tower_id.name)

   
    def generate_seq_for_floors(self):
        if not self.tower_id or not self.tower_id.project_id:
            return
        self._process_activities(self.tower_id.tower_floor_line_id, 'floor', self.tower_id.project_id.name, self.tower_id.name)


    def generate_seq_for_common_area(self):
        if not self.tower_id or not self.tower_id.project_id:
            return
        self._process_activities(self.tower_id.activity_ids, 'common', self.tower_id.project_id.name, self.tower_id.name)

    
    def generate_seq_for_development(self):
        if not self.tower_id or not self.tower_id.project_id:
            return
        self._process_activities(self.tower_id.development_activity_ids, 'development', self.tower_id.project_id.name, self.tower_id.name)
    ...
    #                     for activity_type in activity.activity_type_ids:
    #                         activity_type.index_no = f"{activity_type_seq:03d}"
    #                         activity_type.seq_no = f"VJD/{project_name}/{tower_name}/{str(location_name)}/{activity.name}/{activity_type_seq:03d}"
    #                         activity_type_seq += 1
    ...
    #                     activity_type.index_no = f"{activity_type_seq:03d}"
    #                     if area:
    #                         activity_type.seq_no = f"VJD/{project_name}/{tower_name}/{area}/{activity.name}/{activity_type_seq:03d}"
    #                     else:
    #                         activity_type.seq_no = f"VJD/{project_name}/{tower_name}/{activity.name}/{activity_type_seq:03d}"
    #                     activity_type_seq += 1
    ...
    #                     for activity_type in activity.activity_type_ids:
    #                         activity_type.index_no = f"{activity_type_seq:03d}"
    #                         #_logger.info("------location_name-,%s",str(location_name))
    #                         fs = f"VJD/{project_name}/{tower_name}/{location_name}/{activity.name}/{activity_type_seq:03d}"
    #                         activity_type.seq_no = fs
    #                         #_logger.info("-------FSSSSSSSSSSSSS--,%s",str(fs))
    #                         #_logger.info("---activity_type.seq_noactivity_type.seq_no-,%s",str(activity_type.seq_no))
    #                         activity_type_seq += 1
    ...
    #                         new_number = int(activity_type_seq) + 1
    #                         activity_type_seq = '{:03d}'.format(new_number)
    #                         activity_type.seq_no = "VJD"+"/"+ str(project_name) +"/" + str(tower_name) +"/" + str(activity_name) + "/" + str(temp)
