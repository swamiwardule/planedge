...
class MaterialInspection(models.Model):
    ...
    def button_submit(self,seq_no=None,user_id=None):
        ...
        # 🔥 HIDE maker notification when submit again to checker
        app_log_obj = self.env['app.notification.log'].sudo()
        logs_to_hide = app_log_obj.search([
            ('table_id', '=', self.id),               # Same checklist / MI
            ('res_user_id', '=', self.env.user.id),   # Current checker
    ...
    def button_set_to_maker(self,seq_no,user_id=False):
        ...
        # 🔕 HIDE checker notification when checker rejects and sends back to maker
        app_log_obj = self.env['app.notification.log'].sudo()
        logs_to_hide = app_log_obj.search([
            ('table_id', '=', self.id),               # Same checklist / MI
            ('res_user_id', '=', self.env.user.id),   # Current checker
        ...
        if logs_to_hide:
            logs_to_hide.write({'hide_notification': True})
            _logger.info(
                "Checker notifications hidden: %s",
                logs_to_hide.ids
            )
    ...
    def button_approve(self,seq_no=None,user_id=None):
        ...
        if logs_to_hide:
            logs_to_hide.write({'hide_notification': True})
            _logger.info(
                "Approver notifications hidden: %s",
                logs_to_hide.ids
            )
    ...
    def button_set_to_checker(self,seq_no,user_id):
        ...
        # 🔥 HIDE approver notification when approver rejects and send back to checker
        app_log_obj = self.env['app.notification.log'].sudo()
        logs_to_hide = app_log_obj.search([
            ('table_id', '=', self.id),               
            ('res_user_id', '=', self.env.user.id),   
